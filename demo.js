...
let isInitialTrasnformDone = false;
...
function init_threeScene(spec) {
  ...
  headLoader.load("./models/anonymous/anonymous.json", geometryHead => {
...
function animateAppear(object3D) {
  new TWEEN.Tween(object3D.material).to({ opacity: 1 }, 700).start();
}

function animateDisAppear(object3D) {
  new TWEEN.Tween(object3D.material).to({ opacity: 0 }, 700).start();
}
